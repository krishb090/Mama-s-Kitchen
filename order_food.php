...
.total-section h4 {
    font-size: 1.5rem; /* Larger font size for the total amount */
    color: #333; /* Dark text color for readability */
    font-weight: 600; /* Semi-bold font weight */
    margin-bottom: 10px; /* Space below the heading */
}

.total-section #total_price_display {
    color: #28a745; /* Green color for the total amount */
    font-weight: bold; /* Bold font weight */
}

.total-section input[type="hidden"] {
...
if (isset($_POST['submit_order_food_form']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Insert Order
        $stmt_order = $con->prepare("INSERT INTO placed_orders (order_time, client_id, delivery_address, payment_method, total_price) VALUES (?, ?, ?, ?, ?)");
        $stmt_order->execute([date("Y-m-d H:i"), $client_id, $delivery_address, $payment_method, $total_price]);

        $order_id = $con->lastInsertId();

        // Fetch Menu Names
        $stmtGetMenuNames = $con->prepare("SELECT menu_name FROM menus WHERE menu_id IN (" . implode(',', array_fill(0, count($selected_menus), '?')) . ")");
        $stmtGetMenuNames->execute($selected_menus);
        $menu_names = $stmtGetMenuNames->fetchAll(PDO::FETCH_COLUMN);
        ...
        // Update the order with menu names
        $stmtUpdateOrder = $con->prepare("UPDATE placed_orders SET menu_names = ? WHERE order_id = ?");
        $stmtUpdateOrder->execute([$menu_names_str, $order_id]);

        echo "<div class='alert alert-success'>Great! Your order has been created successfully.</div>";
...
?>
		...
		<form method="post" id="order_food_form" action="order_food.php">
            ...
            <div class="col-sm-12">
                ...
                <div class="invalid-feedback" id="required_payment_method"></div>
            </div>
        </div>
        <div class="form-group colum-row row">
    <div class="col-sm-12 ">
        <h4 style="margin-left: 4px;">Total: $<span id="total_price">0.00</span></h4>
        ...
        function nextPrev(n) 
{
    ...
    // Calculate total before moving to the next tab
    calculateTotal();
    ...
    var total = 0;
    var checkboxes = document.querySelectorAll('input[name="selected_menus[]"]:checked');
    checkboxes.forEach(function(checkbox) {
        var price = parseFloat(checkbox.closest('.itemListElement').querySelector('.menu_price_field span').innerText.replace('$', ''));
        total += price;
    });
